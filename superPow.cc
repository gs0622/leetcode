...
public:
	// ab%k = (a%k)(b%k)%k 
	// (a^b)%k = (a^(b%10))%k * (a^(b/10)%k) %k
	int superPow(int a, vector<int>& b) {
		const int mod=1337;
		function<int(int,int)> mypow = [&](int a, int pw){
			a%=mod;
			int res=1;
			for (int i=0; i<pw; ++i)
				res = (res*a) % mod;
		...
		if (b.empty()) return 1;
		int lsd=b.back(); b.pop_back();
		return mypow(superPow(a,b), 10)*mypow(a, lsd)%mod;
	}
};
int main(){
	Solution s;
