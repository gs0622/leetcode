...
public:
	...
	int swimInWater1(vector<vector<int>>& grid) {
		...
		while (!pq.empty()) {
			...
			for (auto d: dirs) {			// evaluate neighborhood
				int i=r+d[0], j=c+d[1], idx=i*n+j;
				if (i<0 || i>=m || j<0 || j>=n || visit.count(idx)) continue;
